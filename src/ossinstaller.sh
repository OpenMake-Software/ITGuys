...
WEBADMIN=`curl -s http://www.deployhub.org/downloads/rpms/ |grep ".rpm" |  grep "deployhub-webadmin" | cut -d "=" -f2 | cut -d">" -f1 | sed 's/"//g' | head -1`
ENGINE=`curl -s http://www.deployhub.org/downloads/rpms/ |grep ".rpm" |  grep "deployhub-eengine" | cut -d "=" -f2 | cut -d">" -f1 | sed 's/"//g' | head -1`

WEBADMIN='deployhub-webadmin-8.0.0-1.x86_64.rpm'
ENGINE='deployhub-engine-8.0.0-1.x86_64.rpm'

echo Downloading $ENGINE
curl -m 600 -s -L http://www.deployhub.org/downloads/rpms/$ENGINE -o $ENGINE

echo Downloading $WEBADMIN
curl -m 600 -s -L http://www.deployhub.org/downloads/rpms/$WEBADMIN -o $WEBADMIN
